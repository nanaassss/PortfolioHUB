# PortfolioHUB - Geovanna

## 🎯 Objetivo

Bem-vindo ao **PortfolioHUB**! Este é o meu portfólio inicial como estudante de **Análise e Desenvolvimento de Sistemas** (1º Semestre).

O objetivo deste repositório é documentar minha evolução acadêmica e os conhecimentos fundamentais que estou adquirindo nas disciplinas base da computação.

---

## 📁 Estrutura Acadêmica

O repositório está organizado de acordo com as disciplinas que estou cursando:

### 1. 💻 Introdução à Computação
- Fundamentos de hardware e software
- História da computação e sistemas operacionais
- Representação de dados (binário, hexadecimal)

### 2. ⚙️ Engenharia de Software
- Conceitos de ciclo de vida de software
- Processos e metodologias iniciais
- Qualidade e requisitos básicos

### 3. 🧠 Lógica de Programação
- Construção de algoritmos
- Estruturas de controle e repetição
- Práticas iniciais com linguagens de ensino

### 4. 🗄️ Banco de Dados
- Conceitos de dados e informações
- Modelagem conceitual básica
- Introdução aos sistemas gerenciadores

---

## 🛠️ Tecnologias e Ferramentas

Nesta fase inicial, estou focada em:

- **Lógica**: Algoritmos e Fluxogramas
- **Web**: HTML5 e CSS3 (Cursos complementares)
- **Ferramentas**: Git, GitHub, VS Code
- **Bancos de Dados**: Modelagem de Dados

---

## 🌐 Portfólio Web

Este repositório alimenta uma página web moderna que apresenta meu progresso.

**Acesse aqui**: [https://geovanna.github.io/PortfolioHUB](https://geovanna.github.io/PortfolioHUB)

---

## 📧 Contato

- **GitHub**: [github.com/geovanna](https://github.com/geovanna)
- **LinkedIn**: [linkedin.com/in/geovanna](https://linkedin.com/in/geovanna)
- **Email**: geovanna@email.com


## 🔒 Gestão de Usuários e Segurança

Para atender aos critérios de segurança recomendados pelo assistente Google GEMINI, as seguintes políticas foram aplicadas neste projeto:

- **Controle de Acesso (RBAC)**: Permissões de escrita restritas apenas aos administradores do projeto via GitHub Teams.
- **Branch Protection**: A branch principal (`main`/`master`) está protegida contra commits diretos. Alterações só são integradas via *Pull Requests* com aprovação de Code Review.
- **Segurança de Código**: Varredura automática ativada através do GitHub Dependabot para detecção de vulnerabilidades em dependências.
- **Políticas de Segredo**: Utilização de GitHub Secrets para gerenciar quaisquer chaves de API necessárias, evitando exposição de credenciais no código-fonte.

---

**Versão**: 1.0 (Refinada - 1º Semestre)
**Última atualização**: Maio de 2026
